  ...
  describe('migrations', () => {
    ...
    test('regression test of the database state after running migrations, update the snapshot when the database state changes', async () => {
      ...
      expect(await serializeSchema({ db })).toMatchInlineSnapshot(`
        ...
        CREATE INDEX "backup_restore_jobs_organization_id_index" ON "backup_restore_jobs" ("organization_id");
        CREATE INDEX "backup_restore_jobs_status_index" ON "backup_restore_jobs" ("status");
        CREATE INDEX "backup_runs_destination_id_created_at_index" ON "backup_runs" ("destination_id", "created_at");
        CREATE UNIQUE INDEX "backup_runs_single_in_progress_per_destination_index" ON "backup_runs" ("destination_id") WHERE "status" IN ('pending', 'packaging', 'uploading');
        ...
        CREATE TABLE "backup_restore_jobs" ( "id" text PRIMARY KEY NOT NULL, "created_at" integer NOT NULL, "updated_at" integer NOT NULL, "organization_id" text NOT NULL, "destination_id" text, "run_id" text, "source" text NOT NULL, "status" text NOT NULL, "total_documents_count" integer, "processed_documents_count" integer NOT NULL DEFAULT 0, "restored_documents_count" integer, "untrashed_documents_count" integer, "skipped_duplicates_count" integer, "error_message" text, "started_at" integer, "completed_at" integer, "downloaded_bytes" integer, "total_bytes" integer, FOREIGN KEY ("organization_id") REFERENCES "organizations"("id") ON UPDATE cascade ON DELETE cascade, FOREIGN KEY ("destination_id") REFERENCES "backup_destinations"("id") ON UPDATE cascade ON DELETE set null, FOREIGN KEY ("run_id") REFERENCES "backup_runs"("id") ON UPDATE cascade ON DELETE set null );
        CREATE TABLE "backup_runs" ( "id" text PRIMARY KEY NOT NULL, "created_at" integer NOT NULL, "updated_at" integer NOT NULL, "destination_id" text NOT NULL, "organization_id" text NOT NULL, "trigger" text NOT NULL, "status" text NOT NULL, "remote_file_id" text, "remote_file_name" text, "documents_count" integer, "total_size_bytes" integer, "error_message" text, "completed_at" integer, "processed_documents_count" integer NOT NULL DEFAULT 0, "processed_bytes" integer, "total_raw_bytes" integer, "uploaded_bytes" integer, FOREIGN KEY ("destination_id") REFERENCES "backup_destinations"("id") ON UPDATE cascade ON DELETE cascade, FOREIGN KEY ("organization_id") REFERENCES "organizations"("id") ON UPDATE cascade ON DELETE cascade );
